...
def t_test_benf(
    ...
    labels = [
        f"Различия статистически не значимы {0.05/k:.5f} < p:",
        f"Пограничный случай, есть основания задуматься над наличием различий {0.01/k:.5f} < p < {0.05/k:.5f}:",
        f"Различия статистически значимы p < {0.01/k:.5f}.",
    ]
